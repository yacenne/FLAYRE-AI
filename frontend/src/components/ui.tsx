...
export function BrandMark({ compact=false }: { compact?: boolean }) {
  return (
    <div className={`flex items-center gap-2 ${compact ? "" : "text-lg"}`}>
      <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#111111] text-white shadow-sm">
...
export function Button({ className="", variant="primary", ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "primary"|"secondary"|"ghost"; className?: string }) {
  const styles = {
    ...
    secondary: "border border-[#dedede] bg-white text-[#111111] hover:bg-[#f7f7f7]",
    ghost: "text-[#666] hover:bg-[#f3f3f3] hover:text-[#111111]",
  };
...
export function Card({ children, className="", ...props }: HTMLAttributes<HTMLDivElement>) {
  return <div {...props} className={`rounded-2xl border border-[#e7e7e7] bg-white ${className}`}>{children}</div>;
}

export function Label({ children }: { children: ReactNode }) {
  return <div className="mb-2 text-xs font-medium uppercase tracking-[0.08em] text-[#8a8a8a]">{children}</div>;
}
